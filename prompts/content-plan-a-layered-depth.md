# Plan A: "Layered Depth" — ContentSlide v0 Prompt
...
### 2-1. Design Direction: "Layered Depth"
Apple Keynote 스타일. 요소들이 레이어처럼 겹쳐지며 깊이감을 만든다.

### 2-2. Body Text (content 영역)
- 좌측에 tone 색상의 4px vertical bar를 배치
- 본문 텍스트는 bar 옆에 여유로운 padding으로 배치
- text-lg~xl, leading-relaxed, text-gray-600
- content가 빈 문자열이면 이 영역 전체 생략

### 2-3. Bullets 영역 — 핵심 자유 영역 ★
- 각 bullet을 독립적인 **부유 카드(floating card)**로 렌더링
- 카드마다 미세하게 다른 shadow depth로 레이어링 효과
  - 첫 번째: shadow-sm
  - 두 번째: shadow-md (살짝 위로 translate-y)
  - 세 번째 이후: shadow-lg
- 카드 좌측에 tone 색상의 넘버 뱃지 (rounded-full, w-8 h-8)
- 카드 배경: white, border: gray-100
- 카드 간격: space-y-3
- 카드에 hover 없음 (프레젠테이션이므로)
- 최대 5개 bullet 대응
...
[SlideHeader]
   ↓ gap-6
[Body Text with tone bar] (content가 있을 때)
   ↓ gap-6
[Floating Bullet Cards]   (bullets가 있을 때)
   ↓ gap-6
[SlideBottomMessage]       (emphasis가 있을 때)
```
- 전체를 flex-col로 구성, 각 섹션 간 gap-6

### 2-5. Subtle Decorations (선택적)
- 우상단에 tone 색상 원 1개 (w-32 h-32, opacity 3~5%, blur)
- 이것은 분위기용이며 텍스트와 절대 겹치지 않아야 함
